...
class MarkovTweetGeneration:
    def __init__(self, target_user: str) -> None:
        ...
        self.target_id = self.tw_collector.get_user(username=target_user).data["id"]
    ...
    def _text_preperation(self):
        ...
        print(f"Collected at total of {len(tweet_list)} tweets...")
    ...
    def generate_tweet_text(self):
        ...
        try:
            new_sentence = Markov_Model.make_short_sentence(
                min_chars=100, max_chars=150, tries=100
            ).capitalize()
        # TooSmall
        except KeyError:
            return
...
if __name__ == "__main__":
    ...
    writer = MongoWriter()
    print(f"Collecting recent tweets for {user}\n")
