...
class MongoWriter:
    ...
    def insert_twitter_user(self, twitter_id=None, twitter_username=None) -> None:
        ...
        if twitter_id is not None:
            ...
            user_response = self.tw_collector.get_user(username=twitter_username)
        else:
        ...
        try:
            self.connector.user_collection.insert_one(new_twitter_user.to_mongo())
    ...
    def insert_new_tweets(self, twitter_id=None, twitter_username=None, new_user=False):
        if twitter_id is not None:
            ...
            if (
                self.connector.user_collection.find_one({"twitter_id": twitter_id})
                == None
            ):
                self.insert_twitter_user(twitter_id=twitter_id)

        elif twitter_username is not None:
            user_info = self.tw_collector.get_user(username=twitter_username)
            if twitter_username[0] == "@":
                twitter_username = twitter_username[1:]
            if (
                self.connector.user_collection.find_one({"username": twitter_username})
                == None
            ):
        ...
        if (
            self.connector.tweet_collection.find_one({"tweet_id": int(oldest_tweet)})
            != None
        ):
            return
